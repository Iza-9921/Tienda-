...
class AccesoriosViewModel : ViewModel() {
    ...
    fun getById(id: Int): Accesorios? = _accesorios.value.firstOrNull { it.id == id }  // útil para la pantalla de detalle [web:59]

    fun addAccesorios(item: Accesorios) {
        _accesorios.update { it + item }  // ejemplo de mutación de estado inmutable [web:57]
    }

    fun removeById(id: Int) {
        _accesorios.update { it.filterNot { c -> c.id == id } }  // ejemplo de eliminación [web:57]
    }

    fun clickPerson(accesorios: Accesorios) {
        println("Has hecho click en: ${accesorios.TítuloProducto}")  // logging local [web:59]
    }
}
